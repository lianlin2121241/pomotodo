...
(function(factory) {
  ...
  var NiceScrollClass = function(myopt, me) {
          ...
          if (self.opt.cursordragontouch || (!cap.cantouch && !self.opt.touchbehavior)) {
            ...
            if (self.opt.sensitiverail) {
              self.bind(self.rail, "click", function(e) {
                if($(e.srcElement).hasClass("nicescroll-cursors")){//#########放开冒泡后，根据事件触发的dom判断是否为外层panel
                  return;
                }
    ...
    this.getViewport = function(me) {
      ...
      while (dom && (dom.nodeType == 1) && !(/^BODY|HTML/.test(dom.nodeName))) {
        var dd = $(dom);
        if (/fixed|absolute|relative/.test(dd.css("position"))) return dd;//############# if (/fixed|absolute/.test(dd.css("position"))) return dd;
        var ov = dd.css('overflowY') || dd.css('overflowX') || dd.css('overflow') || '';
    ...
    function execScrollWheel(e, hr, chkscroll) {
      ...
      //e.stopImmediatePropagation();//##########取消阻止冒泡
